...
export const createChatRoomWithMembers = async (
  name: string,
  members: string[],
...
export const leaveChatRoom = async (userId: string, chatRoomId: string) => {
...
export const updateLastOnline = (userId: string) => {
  return prisma.user.update({
    where: {
      id: userId,
    },
...
export const markUsersOffline = async () => {
  const threshold = new Date(new Date().getTime() - 5 * 60000);
